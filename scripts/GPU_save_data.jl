...
# here, we will download the lowest resolution dataset (256³) to save time, for the isolated matrix model (IMM). See publication for more details.
data_file = "256_cubed_IMM_compo.jld2"

println("Downloading the dataset from Zenodo, this may take a while...")
...
grt_boundary = convert(CuArray{Bool}, grt_boundary)
...
path_save = "data_model_10_Myr_GPU.h5"  # chose the name and the path of the HDF5 output file (make sure to add .h5 or .hdf5 at the end)
